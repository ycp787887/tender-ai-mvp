...
- **格式壁垒**：各地交易平台专用格式（如.sdtf）无法直接解析

### 1.2 设计目标

> “标书人工比对需要 2-4 小时，这个工具的目标是让一个不懂技术的投标员，上传文件就能获得最完整的投标决策支持”
...
### 3.3 Agent 智能分析模式
早期 Agent 无法区分"用户追问"与"触发分析"两类意图，设计了独立的多层路由判定机制，先做意图分类再决定是否调用工具链。
...
| 打印报告 | 采用浏览器自带打印底层逻辑 | streamlit渲染一键打印为pdf文件排版混乱，字体堆叠 |
| 缓存机制 | 磁盘缓存 | 成本控制 + 响应速度 |
| 历史记录 | SQLite | 轻量级，满足MVP需求 |
| 专用加密格式 | 不破解，引导OCR | 合规优先，技术克制 |
| OCR与文件流产生冲突 | 引入_ocr_done_flag标记跳过格式校验 | 卡了三天调试解决 |
...
| 片段1 | 三公司上传 + 缓存秒出 | 成本控制 + 性能优化 |
| 片段2 | Agent一键分析 + 追问记忆 | 意图识别 + LangGraph + 废标判定 |
| 片段3 | 容灾链路 + 合规设计 | OCR图片上传 + 分析 |
...
[点击观看视频](https://www.bilibili.com/video/BV1MgGE6aE7C/?spm_id_from=333.1387.homepage.video_card.click&vd_source=f8cbd4d47f48a83e723b3b5626dd9756)
...
联系邮箱：3054676878@qq.com  

---

##  文档说明
本文档为《智能投标风险分析系统》的技术设计说明，完整展示系统的业务理解、架构设计、核心实现、技术决策。代码未开源，如有技术沟通或面试演示，欢迎联系我。
